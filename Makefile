.PHONY: app build dmg test validate-release clean
...
test:
	...
	CLANG_MODULE_CACHE_PATH="$(CURDIR)/.build/ModuleCache" xcrun swiftc -parse-as-library -target "$$(uname -m)-apple-macosx13.0" -framework AppKit -framework ApplicationServices -framework CoreGraphics -framework ServiceManagement Sources/ScrollSplit/App/ApplicationLaunchContext.swift Sources/ScrollSplit/App/ReverseScrollingController.swift Sources/ScrollSplit/Settings/AppSettings.swift Sources/ScrollSplit/Scrolling/MouseScrollAction.swift Sources/ScrollSplit/Scrolling/ScrollSourceClassifier.swift Sources/ScrollSplit/Scrolling/ScrollEventTap.swift Sources/ScrollSplit/Services/PermissionService.swift Sources/ScrollSplit/Services/LoginItemService.swift Tests/Smoke/RuntimeStateSmoke.swift -o "$(CURDIR)/.build/tests/RuntimeStateSmoke"
